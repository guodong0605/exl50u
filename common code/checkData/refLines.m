function refLines(figHandle)
    ...
    for i = 1:length(axHandles)
        ...
        % 插入两条参考线
        refLines(i*2-1) = line(ax, [xlims(1), xlims(1)], get(ax, 'YLim'), 'Color', 'red', 'LineStyle', '--', 'Tag', 'RefLine1','LineWidth',2);
        refLines(i*2) = line(ax, [xlims(2), xlims(2)], get(ax, 'YLim'), 'Color', 'blue', 'LineStyle', '--', 'Tag', 'RefLine2','LineWidth',2);
        
        % 添加dx文本
        dxText(i) = text(ax, xlims(2), mean(get(ax, 'YLim')), sprintf('x1: %.2f\nx2: %.2f\ndx: %.2f', xlims(1), xlims(2), diff(xlims)), 'HorizontalAlignment', 'left', 'VerticalAlignment', 'bottom');
        hold(ax, 'off');
    end
