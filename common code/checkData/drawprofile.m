function drawprofile(shotnum,chnnames,acq_start,Tprofile,isShift,figtype)
...
downsampleRate=1;
holdPlot=1;
if (nargin <3) || isempty(Tprofile), Tprofile = Tprofile_default; end
if (nargin <4) || isempty(isShift), isShift = isShift_default; end
if (nargin <5) || isempty(figtype), figtype = figtype_default; end

dt=0.05;       % used to do the average of the raw data,  the average value=mean(t+dt)
...
setappdata(0,'fig_std',fig_std);
setappdata(0,'holdPlot',holdPlot);

%%
%--------------------draw the data figure-----------------------------------
f1=figure;set (f1,'Units','normalized','color','w'); % figure 1
...
if data_profile
    ...
    diagName = regexp(chnname{1}, '^[a-zA-Z]+', 'match');
    switch lower(diagName{1})
        case 'axuv'
            axuvparam=getAXUVcoef();
            num2=axuvparam.axuv_z800(num);
        otherwise
            num2=num;
    end
    figure('Color',[1 1 1],'name','dataprofile');
    axe2=plot(num2,data_mean,'--o','LineWidth',2.5,'MarkerSize',8,'MarkerFaceColor',[1 0 0 ],'MarkerEdgeColor',[1 0 0 ]);
    xlabel('Channel');
    ylabel(diagName{1});
    ...
    legend( [' @',num2str(round((t1+t2)/2*1e3)),'ms',',\delta t=',num2str(round((t2-t1)*1000))])
    setappdata(0,'num',num);
    if fig_std
        figure('Color',[1 1 1],'name','stdprofile');
        axe2=plot(num2,data_std,'--s','LineWidth',2.5,'MarkerSize',8,'MarkerFaceColor',[1 0 0 ],'MarkerEdgeColor',[1 0 0 ]);
        xlabel('CHN');
        ylabel('STD(au)');
        set(gca, 'FontName',   'Times New Roman', 'FontUnits',  'points','FontSize',  15, 'FontWeight', 'bold', 'LineWidth', 1.5, 'XMinorTick', 'on', 'YMinorTick', 'on','ticklength',[0.02 0.02],'Xgrid','on');
        legend(['profile @',num2str(t1+dt/2),'s'])
        setappdata(0,'num',num2);
    end
    ...
    function allevents(src,evt)
        ...
        fig_std=getappdata(0,'fig_std');
        holdPlot=getappdata(0,'holdPlot');
        ...
        if figtype==1   %如果是线条
            ...
            thisline2=thisline(end:-1:1);
            for i=1:length(thisline2)
                temp=thisline2(i);
        ...
        if ~isempty(findfig)
            ...
            if holdPlot
                oldString=ax.Legend.String;
                hold on;plot(num2,mean_y,'--o','LineWidth',2.5,'MarkerSize',8)                
                oldString{end+1}=[' @',num2str(round((t1+t2)/2*1e3)),'ms',',\delta t=',num2str(round((t2-t1)*1000))];
                ax.Legend.String=oldString;
            else
                ax.Legend.String=[' @',num2str(round((t1+t2)/2*1e3)),'ms',',\delta t=',num2str(round((t2-t1)*1000))];
                ax.Children.YData= mean_y;
            end
        else
            figure('Color',[1 1 1],'name','dataprofile');
            axe2=plot(num2,mean_y,'--o','LineWidth',2.5,'MarkerSize',8,'MarkerFaceColor',[1 0 0 ],'MarkerEdgeColor',[1 0 0 ]);
            set(gca, 'FontName',   'Times New Roman', 'FontUnits',  'points','FontSize',  15, 'FontWeight', 'bold', 'LineWidth', 1.5, 'XMinorTick', 'on', 'YMinorTick', 'on','ticklength',[0.02 0.02],'Xgrid','on');
