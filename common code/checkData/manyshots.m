function [allData,time]=manyshots(shotnums, chn,Tstart,Tend,Fs,figflag)

% This function downloads and plots multiple shots of data for multiple channels.
%   It takes an optional input argument 'isShift' which is a flag indicating whether to shift the data.
%   By default, the value of 'isShift' is 0, meaning the data will not be shifted.
%   If 'isShift' is set to 1, the data will be shifted.
% Example usage:

% manyshots('3001:3003', 'ip,hcn_ne001,i_cs,ha009',-0.3,4)
...
% 初始化三维数组
allData = nan(shotnum, chnnum, datalength);
allTime=allData;
% 循环下载数据并填充到三维数组中
for k = 1:shotnum
    for n = 1:chnnum
        [channelData,time,unit] = downloaddata(shots(k), chns{n},datatime,0,isShift_default);
    ...
    for i = 1:chnnum   %根据通道的数量进行确定subplot的数量
        ...
        for j = 1:shotnum
            ...
            if j==shotnum
                ...
                if unitflag
                    ylabel(['k',unitStr{i}],'Interpreter','none','fontSize',ylabelfontsize,'interpreter','tex')
                else
                    ylabel(unitStr{i},'Interpreter','none','fontSize',ylabelfontsize,'interpreter','tex')
                end
